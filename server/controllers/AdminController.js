...
res.cookie('admin_token', token, {
  ...
  path: '/',              // Important: make it accessible everywhere
});
...
export const fineEducatorandcourse = async (req, res) => {
  try {
    ...
    // Get all unique educator IDs (stored as strings in course.educator)
    const educatorIds = [...new Set(courses.map(course => course.educator).filter(Boolean))];

    // Convert string IDs to ObjectIds for querying
    const { ObjectId } = mongoose.Types;
    const validEducatorIds = educatorIds
      .filter(id => ObjectId.isValid(id))
      .map(id => new ObjectId(id));

    // Fetch all educators in a single query
    const educators = await User.find({ 
      _id: { $in: validEducatorIds } 
    }).lean();

    // Create a map for quick educator lookup by _id string
    const educatorMap = new Map();
    educators.forEach(educator => {
      educatorMap.set(educator._id.toString(), educator);
    });

    // Fetch all purchases for all courses in a single query
    const courseIds = courses.map(course => course._id);
    const purchases = await Purchase.find({ 
      courseId: { $in: courseIds } 
    }).lean();

    // Group purchases by courseId for efficient lookup
    ...
    // Format the response data with educator info
    const data = courses.map(course => {
      // Find educator from map using course.educator (string _id)
      const educator = educatorMap.get(course.educator) || null;
      ...
      return {
        course: {
          ...
          totalDuration: course.formattedDuration || course.totalDuration,
        ...
        purchases: purchasesByCourse[course._id.toString()] || []
      };
    });
    ...
    console.error("Error in fineEducatorandcourse:", error);
...
export const approveEducator = async (req, res) => {
  try {
    ...
    // 1. Find and update the Educator application
    ...
    // 2. Find the User by the educator's userId and update their role
    const user = await User.findByIdAndUpdate(
      educator.userId,  // This should match the User._id
      { 
        role: "educator",
        // Optionally store educator reference
        $set: { educatorProfile: educator._id }
      },
      { new: true }
    );
    ...
    res.json({ 
      success: true, 
      message: "Educator approved and user role updated!",
      data: {
        educator,
        user: {
          _id: user._id,
...
export const getCoursesForVerification = async (req, res, filters = {}) => {
  try {
    ...
    // Fetch all unique educator IDs first
    const educatorIds = [...new Set(courses.map(c => c.educator).filter(id => id))];

    let educatorMap = {};

    // Fetch educators from User model using _id
    if (educatorIds.length > 0) {
      try {
        // Batch fetch all educators from User model
        const users = await User.find({ _id: { $in: educatorIds } }).lean();

        users.forEach(user => {
          educatorMap[user._id.toString()] = {
            id: user._id,
            name: user.name || 'Unknown Educator',
        ...
        console.error('User model batch fetch failed:', dbError);
      }
    }

    const enrichedCourses = courses.map(course => {
      const educator = educatorMap[course.educator?.toString()] || {
        id: course.educator,
        name: course.educatorInfo?.name || 'Unknown Educator',
        email: course.educatorInfo?.email || 'No email',
        profileImage: course.educatorInfo?.profileImage || null,
        verified: false,
      };
...
export const getCourseDetailsForVerification = async (req, res) => {
  try {
    ...
    // === EDUCATOR DETAILS (from User model using _id) ===
    let educatorDetails = {
      id: course.educator,
    ...
    try {
      // Query User model using _id (educator field is MongoDB ObjectId)
      ...
      console.warn(`Failed to fetch educator ${course.educator} from User model`);
    }
    ...
    const enrolledStudentsDetails = await Promise.all(
      (course.enrolledStudents || []).map(async (studentId) => {
        ...
        try {
          ...
          console.warn(`Failed to fetch student ${studentId} from User model`);
        }
        return info;
      })
    );
      ...
      (course.courseRatings || []).map(async (rating) => {
        ...
        try {
          ...
          if (dbUser) {
            info = {
              ...info,
              name: dbUser.name || 'Unknown Reviewer',
              email: dbUser.email || 'No email',
              profileImage: dbUser.imageUrl || null,
            };
          }
        } catch (e) {
          console.warn(`Failed to fetch reviewer ${rating.userId} from User model`);
        }
        return info;
      })
    );
